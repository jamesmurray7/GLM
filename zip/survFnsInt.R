...
#' Special case for the ZIP model fits, where intercept-only REs are used.
#' ---
#' These are:
#' GLOBAL:::
...
#' SUBJECT-SPECIFIC:::
#' Fu: design matrix on all failure times survived by subject i (a vector of ones)
...
surv.mod <- function(cph, data, l0.init = NULL){
  ...
  for(i in uids){
    ...
    Fu[[i]] <- cbind(rep(1, length(l0u[[i]])), rep(1, length(l0u[[i]])))
