...
Longit.inits <- function(data){
  fit <- glmmTMB(as.formula(paste0('Y ~ time + cont + bin + (1|id)')),
...
TimeVarCox <- function(data, REs){
  ...
  gamma <- matrix(0, nrow(ss2), 1)

  gamma[,1] <- ss2[,'po_(intercept)'] + ss2[,'zi_(intercept)']
  ...
  ph <- coxph(Surv(survtime, status) ~ cont + bin + gamma, data = ss3[ss3$time1 == 0, ])
