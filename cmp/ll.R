...
.bfits <- mapply(function(b, X, Y, Z, G){
  u <- ucminf::ucminf(b, .ll, Score_b,  # NULL grad(b) function until can work out what this should be!
...
Sigmai <- mapply(function(mu, V, Z){
  nr <- nrow(Z); nc <- ncol(Z)
  out <- matrix(0, nc, nc)
  for(i in 1:nr){
    out <- out + mu[i,]/V[i,] * tcrossprod(Z[i,])
  }
  solve(out)
}, mu = mu, V = V, Z = Z, SIMPLIFY = F)
...
V2 <- mapply(Var_means, lambda, nu, logZ_, summax = 10, SIMPLIFY = F)

# D update ----
Drhs <- mapply(function(b, S){
  S + tcrossprod(b)
}, b = b.hat, S = W_b, SIMPLIFY = F)
