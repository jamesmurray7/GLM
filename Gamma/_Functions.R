...
createDataMatrices <- function(data, formulas){
  ...
  list(
    X = X, Y = Y, Z = Z,
    p = ncol(X[[1]]), q = ncol(Z[[1]]),     # Return dimensions + the matrices.
    np = colnames(X[[1]]), nq = colnames(Z[[1]])
  )
}
...
log.lik <- function(Omega, dmats, b, surv, sv, l0u, l0i){
  ...
  Fu <- sv$Fu; Fi <- sv$Fi; 
  q <- dmats$q; P <- dmats$p; n <- surv$n
  
  #' f(Y|...; Omega)
  ll.G <- mapply(function(Y, X, Z, b){
    mu <- exp(X %*% beta + Z %*% b)
    ll_Gamma2(Y, shape, mu)
  }, Y = Y, X = X, Z = Z, b = b)
  ...
  df <- P + Ps + 1                             #' All __estimated__ parameters (1 := Shape).
...
my.summary <- function(myfit, printD = F){
  ...
  cat(sprintf("Shape parameter for Gamma distribution: %.3f.\n", shape))
  
  beta <- setNames(c(betas), row.names(betas))
  my <- c(beta)
  
  rSE <- SE[grepl('^beta', names(SE))]
