...
simData <- function(n = 250, ntms = 10, fup = 5,
                    beta = c(1, 0.10, 0.33, -0.50), alpha = c(-0.1, 0.2),
                    D = matrix(c(0.5, 0, 0, 0.1), 2, 2), 
                    gamma = 0.5, zeta = c(0.05, -0.30), 
                    dispFormula = ~ time,
                    theta = c(-4, 0.2), cens.rate = exp(-3.5)){
  #' Necessary parameters & data generation ----
  time <- seq(0,fup, length.out = ntms); tau <- fup + 0.1 # time variable and truncation time tau.
