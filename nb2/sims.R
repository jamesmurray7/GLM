...
for(i in 1:100){
  ...
  fits3[[i]] <- tryCatch(suppressMessages(EM(data3[[i]]$data, ph3, data3[[i]]$surv.data, verbose = F, gh = 3)),
                         error = function(e) NULL)
  fits4[[i]] <- tryCatch(suppressMessages(EM(data4[[i]]$data, ph4, data4[[i]]$surv.data, verbose = F, gh = 3)),
                         error = function(e) NULL)
  fits5[[i]] <- tryCatch(suppressMessages(EM(data5[[i]]$data, ph5, data5[[i]]$surv.data, verbose = F, gh = 3)),
                         error = function(e) NULL)
  fits6[[i]] <- tryCatch(suppressMessages(EM(data6[[i]]$data, ph6, data6[[i]]$surv.data, verbose = F, gh = 3)),
                         error = function(e) NULL)
  utils::setTxtProgressBar(pb, i)
}
fits <- list(fits1, fits2, fits3, fits4, fits5, fits6)
save(fits, file = '~/Downloads/nbfits2.RData')