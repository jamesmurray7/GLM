...
b.minimise <- function(b, X, Y, lY, Z, delta, S, SS, Fi, Fu, l0i, l0u, Delta, 
  ...
  if(obj == 'joint_density'){
    if(method == 'optim'){
      b.hat <- mapply(function(b, X, Y, lY, Z, delta, S, SS, Fi, Fu, l0i, l0u, Delta){
    ...
    if(Hessian == 'obj'){
      Sigma <- mapply(function(b, X, Y, lY, Z, delta, S, SS, Fi, Fu, l0i, l0u, Delta){
    ...
    if(method == 'optim'){
      b.hat <- mapply(function(b, X, Y, lY, Z, delta){
        optim(b, marginal_Y, marginal_Y_db,
              X = X, Y = Y, lY = lY, Z = Z, beta = beta, delta = delta, D = D,
              summax = summax, method = 'BFGS')$par
      }, b = b, X = X, Y = Y, lY = lY, Z = Z, delta = delta, SIMPLIFY = F)
    }else{
      b.hat <- mapply(function(b, X, Y, lY, Z, delta){
