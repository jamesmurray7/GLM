...
// The "raw" score for beta using quadrature, but not forward differencing.
// [[Rcpp::export]]
vec Sbeta2(vec& beta, vec& b, mat& X, mat& Z, vec& Y, vec& lY, 
  ...
  for(int l = 0; l < gh; l++){
    ...
    // Calculate the Score at this quad node.
...
List delta_update(List delta, List b, List X, List Z, List Y, List lY,
  ...
  for(auto num = inds.begin(); num != inds.end(); num++){
    ...
    double Hd_i = Hdelta(delta_i, b_i, X_i, Z_i, Y_i, lY_i, beta, tau_i, w, v, summax, 0.0005610352);// pow(datum::eps, 1./4.));
