...
double E_llcmp_beta(vec beta, mat& G, vec& b, mat& X, mat& Z, vec& Y, vec& lY, vec& delta, vec& tau, vec& w, vec& v, int summax){
  // nu unchanging
  vec nu = exp(G * delta);
  vec eta = X * beta + Z * b;
  // init output
...
// [[Rcpp::export]]
vec Sbeta_cdiff(vec beta, mat& G, vec& b, mat& X, mat& Z, vec& Y, vec& lY, vec& delta, vec& tau, vec& w, vec& v, int summax, double eps){
  int a = beta.size();
  vec out = vec(a);
  for(int i = 0; i < a; i++){
    vec aa = beta, bb = beta;
    ...
    aa[i] = beta[i] + eps * xi;
    bb[i] = beta[i] - eps * xi;
    double E1 = E_llcmp_beta(aa, G, b, X, Z, Y, lY, delta, tau, w, v, summax);
    double E2 = E_llcmp_beta(bb, G, b, X, Z, Y, lY, delta, tau, w, v, summax);
...
// [[Rcpp::export]]
mat Hbeta(vec beta, mat& G, vec& b, mat& X, mat& Z, vec& Y, vec& lY, vec& delta, vec& tau, vec& w, vec& v, int summax, double eps){
  int n = beta.size();
  
  if(n == 1){
    double val = (E_llcmp_beta(beta + eps, G, b, X, Z, Y, lY, delta, tau, w, v, summax) - 
      2.0 * E_llcmp_beta(beta, G, b, X, Z, Y, lY, delta, tau, w, v, summax) + 
      E_llcmp_beta(beta - eps, G, b, X, Z, Y, lY, delta, tau, w, v, summax))/pow(eps, 2.0);
    mat H = mat(1, 1, fill::value(val));
    return H;
  }else{
    ...
    for(int i = 0; i < n; i++){
      vec ei = ee.col(i);
      // Diagonal terms
      H(i, i) = (E_llcmp_beta(beta - ei, G, b, X, Z, Y, lY, delta, tau, w, v, summax) - 
        2.0 * E_llcmp_beta(beta, G, b, X, Z, Y, lY, delta, tau, w, v, summax) + 
        E_llcmp_beta(beta + ei, G, b, X, Z, Y, lY, delta, tau, w, v, summax))/pow(eps, 2.0);
      for(int j = (i + 1); j < n; j++){
        vec ej = ee.col(j); // Off-diagonals
        H(i,j) =  (E_llcmp_beta(beta + ei + ej, G, b, X, Z, Y, lY, delta, tau, w, v, summax) - 
          E_llcmp_beta(beta + ei - ej, G, b, X, Z, Y, lY, delta, tau, w, v, summax) - 
          E_llcmp_beta(beta - ei + ej, G, b, X, Z, Y, lY, delta, tau, w, v, summax) + 
          E_llcmp_beta(beta - ei - ej, G, b, X, Z, Y, lY, delta, tau, w, v, summax))/(4.0 * pow(eps, 2.0));
        H(j,i) = H(i,j);
      }
    }
    return H;
