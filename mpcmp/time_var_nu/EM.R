...
EMupdate <- function(Omega, X, Y, lY, Z, W, delta, b, S, SS, Fi, Fu, l0i, l0u, Delta, l0, 
  ...
  if(beta.update.quad){ #' Taken **with** quadrature
    ...
    }, b = b.hat, X = X, Z = Z, W = W, Y = Y, lY = lY, delta = delta, tau = tau, summax = summax, SIMPLIFY = F)
    ...
    }, b = b.hat, X = X, Z = Z, W = W, Y = Y, lY = lY, delta = delta, summax = summax, SIMPLIFY = F)
  ...
  delta.new <- lapply(1:n, function(i){ # (All-in-one function!)
    # print(i)
    if(i %in% inds.met)
      return(delta.update(delta[[i]], X[[i]], Z[[i]], W[[i]], Y[[i]],
...
EM <- function(long.formula, surv.formula, disp.formula,
  ...
  }else{                 #' Otherwise set delta_i = c(0, 0, ...) \forall i = 1, ..., n.
  ...
  while(diff > tol && iter < maxit){
    s.em.i <- proc.time()[3]
    update <- EMupdate(Omega, X, Y, lY, Z, W, delta, b, S, SS, Fi, Fu, l0i, l0u, Delta, l0, 
