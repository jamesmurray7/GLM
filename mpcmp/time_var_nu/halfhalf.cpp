...
// [[Rcpp::export]]
double joint_density(vec& b, mat& X, vec& Y, vec& lY, mat& Z, mat& W,       // Data matrices
  ...
  vec loglambda = log(lambda);
  vec logZ = logZ_c(loglambda, nu, summax);
  // Calculate loglik CMP and thenkdasdajdjffor other consituent distns.
  double ll = ll_cmp(loglambda, nu, logZ, Y, lY);
  int q = b.size();
  double temp = 0.0;
...
// [[Rcpp::export]]
vec joint_density_ddb(vec& b, mat& X, vec& Y, vec& lY, mat& Z, mat& W,       // Data matrices
                      vec& beta, vec& delta, mat& D,                        // Longit. + RE parameters
                      rowvec& S, mat& SS, rowvec& Fi, mat& Fu, double l0i, rowvec& haz,
                      int Delta, double gamma, vec& zeta, int summax){
  // Define mu, nu
...
// [[Rcpp::export]]
mat joint_density_sdb(vec& b, mat& X, vec& Y, vec& lY, mat& Z, mat& W,       // Data matrices
                      vec& beta, vec& delta, mat& D,                       // Longit. + RE parameters
                      rowvec& S, mat& SS, rowvec& Fi, mat& Fu, double l0i, rowvec& haz,
  ...
  // Rcout << "0------------" << std::endl;
  vec f0 = joint_density_ddb(b, X, Y, lY, Z, W, beta, delta, D, S, SS, Fi, Fu, l0i, haz, Delta, gamma, zeta, summax);
  for(int i = 0; i < q; i++){
    // Rcout << q << "-------------" << std::endl;
    vec bb = b;
    double xi = std::max(1.0, b[i]);
    bb[i] = b[i] + (xi * eps);
    vec fdiff = joint_density_ddb(bb, X, Y, lY, Z, W, beta, delta, D, S, SS, Fi, Fu, l0i, haz, Delta, gamma, zeta, summax) - f0;
...
// [[Rcpp::export]]
vec Sbeta2(vec& beta, vec& b, mat& X, mat& Z, mat& W, vec& Y, vec& lY,
...
// [[Rcpp::export]]
mat Hbeta2(vec& beta, vec& b, mat& X, mat& Z, mat& W, vec& Y, vec& lY,
  vec& delta, vec& tau, vec& w, vec& v, int summax){
  int P = beta.size(), gh = w.size();
...
mat getW2(vec& A, vec& VY, vec& VlY, mat& W, vec& nu){
	...
	// Rcout << "W2" << W2 << std::endl;
	for(int m = 0; m < mi; m++){
		rowvec Wm = W.row(m);
		// colvec WmT = Wm.t();
		// Rcout << "Wm" << WmT << std::endl;
		// Rcout << "First part: " << -A2.at(m) / -VY.at(m) + VlY.at(m) << std::endl;
		// double first_part = -A2.at(m) / VY.at(m) + VlY.at(m);
		// double first_part2 = first_part * nu2.at(m);
		// Rcout << "First part2: " << first_part2 << std::endl;
		// vec Wmfp = first_part2 * WmT;
		// Rcout << "First part3: " << Wmfp << std::endl;
