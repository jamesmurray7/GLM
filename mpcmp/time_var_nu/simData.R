...
simData_joint2 <- function(n = 250, ntms = 10, summax = 100,  fup = 3, 
  ...
  #' Simulate CMP response //
  eta <- X %*% beta + rowSums(Z * b[df$id, ]) # Linear predictor
  deltas.int <- sample(delta.int, n, T)               # Sample dispersion from c(x, y))
  deltas.time <- sample(delta.time, n, T)               # Sample dispersion from c(x, y))
...
# "Safer" version of simdata2 ---------------------------------------------
simData_joint2_safe <- function(n = 250, ntms = 10, summax = 100,  fup = 3, 
                                beta = c(2, -0.1, 0.1, -0.2),               # (int, time, cont, bin)
                                delta.int = c(1.0, -1.0),  
                                delta.time = c(0.1, -0.1),
                                D = matrix(c(0.25, 0, 0, 0.00), 2, 2), 
