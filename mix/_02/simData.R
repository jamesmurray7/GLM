#' #######
#' Simulates data under a Mixture of continuous, binary and continuous, which are linked to a 
...
true.beta <- rbind(c(0.00, 1.00, 0.50, -0.25),    # Gaussian
                   ...
                   c(0.00, 0.20, -0.25, 0.30))    # Gaussian
# Covariance matrix D
true.D <- as.matrix(Matrix::bdiag(
  matrix(c(0.5^2, 0.1, 0.1, 0.25^2), 2, 2),
  matrix(c(0.6^2, -0.05, -0.05, 0.15^2), 2, 2),
  matrix(c(0.5^2, 0.1, 0.1, 0.25^2), 2, 2)
))
true.D[5:6, 1:2] <- true.D[1:2, 5:6] <- matrix(c(0.200, 0.010, 0.010, 0.100))
true.D[3:4, 1:2] <- true.D[1:2, 3:4] <- matrix(c(0.005, 0.000, 0.000, 0.001))
true.D <- as.matrix(Matrix::nearPD(true.D, keepDiag = T)$mat)
# Survival
true.gamma <- c(0.50, -0.25, 0.40)
true.eta <- c(0.05, -0.30)

simData <- function(n = 250, ntms = 10, beta = true.beta, D = true.D, var.e = c(0.25, 0.25)){
  ...
  df$Y.1 <- eta1 + rnorm(n * ntms, sd = sqrt(var.e[1]))
  df$Y.2 <- rbinom(n * ntms, 1, plogis(eta2))
  df$Y.3 <- eta3 + rnorm(n * ntms, sd = sqrt(var.e[2]))
  df
}

simData_joint <- function(n = 250, ntms = 10, beta = true.beta, var.e = c(0.25, 0.25), D = true.D, 
  ...
  df$Y.1 <- c(eta1 + rnorm(n * ntms, sd = sqrt(var.e[1])))
  df$Y.2 <- rbinom(n * ntms, 1, plogis(eta2))
  df$Y.3 <- c(eta3 + rnorm(n * ntms, sd = sqrt(var.e[2])))
