#' ####
#' Testing dynsurv 2 (trying to wrap in a function for id and u)
#' ####
...
# joineRML block ----------------------------------------------------------
# (for comparing, are these quantiles sensible?)
library(joineRML)
...
newdata <- data %>% filter(id == 1, time < 9)
joineRML::dynSurv(mj, newdata, u = 9, type = 'simulated', M = 200)
dynSurv(fit, data, id = 1, u = 2)
dynSurv(fit, data, id = 1, u = 9, b.method = 'MH')

# For someone who fails
data %>% filter(status == 1) %>% distinct(id) %>% pull -> fails
data %>% filter(id %in% fails)
newdata <- data %>% filter(id == 3, time < 8)
joineRML::dynSurv(mj, newdata, u = 8, type = 'simulated', M = 200)
# Vector of candidate times
ds <- dynSurv(fit, data, id = 3, u = c(3,4,5,6,7,8))
for(i in c(3,4,5,6,7,8)){
  newdata <- data %>% filter(id ==3, time < i)
  mjds <- joineRML::dynSurv(mj, newdata, u = i, type = 'simulated', M = 200, progress = F)
  print(mjds)
}
