...
surv.mod <- function(cph, data, l0.init = NULL){
  ...
  Fi <- matrix(NA, nr = length(uids), nc = 3)
  Di <- l0i <- c()
  ...
  for(i in uids){
    ...
    Fi[i,] <- c(1, unique(i.dat$survtime), unique(i.dat$survtime)^2)
    Fu[[i]] <- cbind(1, sf$time[which(sf$time <= unique(i.dat$survtime))], sf$time[which(sf$time <= unique(i.dat$survtime))]^2)
    ...
    if(Di[i] == 0 & unique(i.dat$survtime) <= min(sf$time)){ l0u[[i]] <- 0; Fu[[i]] <- cbind(1, 0, 0) }
  }
