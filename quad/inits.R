...
Longit.inits <- function(K, data){
  lfitK <- list()
  for(k in 1:K){
    lfitK[[k]] <- lme(fixed = as.formula(paste0('Y.', k, '~ time + I(time^2) + cont + bin')),
                      random = as.formula(paste0( ' ~ time + I(time^2) |id')), data = data,
                      method = "ML",
...
TimeVarCox <- function(data, REs, df = 3, fixef.surv = c('cont', 'bin'),
  ...
  for(k in 1:K){
    ssK <- ss2[, c('time1', paste0(c('intercept_', 'slope_', "quad_"), k))]
    gammaK[, k] <- ssK[, 2] + ssK[, 1] * ssK[, 3] + ssK[, 1]^2 * ssK[, 4]
  }
