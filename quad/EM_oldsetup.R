...
#'                    (i.e. ~/.../ApproximateEM/quadratic)
#' ----
#' This is for K = 3, under a QUADRATIC RE structure (q=9)
#' corresponding programs exist for K=5, and K=10, #### NYI ####
...
em <- function(data, ph,
  ...
  b0 <- matrix(NA, nr = length(uids), nc = nK * 3)
  for(i in uids){
  ...
  names(params) <- c(rep("D", length(vech(D))), "var.e_1", "var.e_2", "var.e_3",
                     paste0("gamma_", 1:3),
                     paste0(rep(paste0("beta", 1:nK), each = 5), "_", 0:4),
  ...
  while(diff > tol & iter < max.iter){
    ...
    bi.mat <- matrix(NA, nrow = length(uids), nc = nK * 3)
    ...
    for(i in uids){
      ...
      bi.mat[i,] <- bi; 
      bb <- matrix(bi, nr = nK, byrow = T)
      # And the variance
      Sigmai.store[[i]] <- Sigmai
      S <- list()
      # Covariance matrices on the first, second and third REs
      S[[1]] <- Sigmai[1:3,1:3]; S[[2]] <- Sigmai[4:6, 4:6]; S[[3]] <- Sigmai[7:9, 7:9]
      ...
      mu.surv <- exp(K[[i]] %*% eta) %x% exp(Fu[[i]] %*% (gamma[1] * bb[1,] + gamma[2] * bb[2,] + gamma[3] * bb[3,]))
    ...
    # \lambda_0
    SS <- lapply(Sigmai.store, function(x) lapply(split(seq(nK * 3), rep(1:nK, each = 3)), function(y) x[y,y]))
    lambda <- lambdaUpdate(surv.times, ft, gamma, eta, K, SS,
                           bi.mat[, 1:3], bi.mat[, 4:6], bi.mat[, 7:9],
                           n, w, v, gh.nodes)
