...
getXi <- function(data, K = 5){
  ...
  for(i in uids){
    ...
    Xi <- list()
    for(j in 1:K){
      Xi[[j]] <- cbind(1, i.dat$time, i.dat$time^2, i.dat$cont, i.dat$bin)
    }
...
getZi <- function(data, K = 5){
  ...
  for(i in uids){
    ...
    for(j in 1:K) Zi[[j]] <- cbind(1, i.dat$time, i.dat$time^2)
...
splitXks <- function(data, K = 5){
  ...
  for(i in uids){
    ...
    i.dat <- subset(data, id == i)
    Xi <- cbind(1, i.dat$time, i.dat$time^2, i.dat$cont, i.dat$bin)
...
splitZks <- function(data, K = 5){
  ...
  for(i in uids){
    Zik <- list()
    Zi <- cbind(1, data[data$id == i, "time"], data[data$id == i, "time"]^2)
