...
rm(list=ls())
source('grid-simData.R')
data <- simData_joint(n = 250, delta = c(0.8,0.0), ntms = 10, theta = c(-3, .25), fup = 3,
                      beta = c(-0.5, -0.1, 0.05, -0.1), gamma = 0.6, zeta= c(0.0, -0.2),
...
fit <- glmmTMB(long.formula, data$data, poisson)
fit.trunc <- glmmTMB(long.formula, data$data, glmmTMB::truncated_poisson, ziformula = ~1)
fit.zi <- glmmTMB(long.formula, data$data, poisson, ziformula = ~1)
fit.nb <- glmmTMB(long.formula, data$data, glmmTMB::truncated_nbinom2, ziformula = ~1)
VarCorr(fit)$cond$id
VarCorr(fit.trunc)$cond$id
VarCorr(fit.zi)$cond$id
VarCorr(fit.nb)$cond$id
