#' #####
#' Suite of helper functions for binomial sub-model
#' #####

simData <- function(n = 250, ntms = 10, beta = c(1, 0.10, 0.33, -0.5),
  ...
  message(round(100 * sum(df$Y==1)/length(df$Y), 2), '% incidence rate')
  df
}

simData_joint <- function(n = 250, ntms = 10, beta = c(1, 0.10, 0.33, -0.5),
                          D = matrix(c(0.5, 0, 0, 0.1), 2, 2),
