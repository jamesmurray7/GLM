...
.unpack <- function(x){ # x a list of lists.
  for(i in 1:3){
    ...
    message('\nSaving in ', save.location, 'binests-', i, '.RData')
    save(out, file = paste0(save.location, 'binests-', i, '.RData'))
  }
}
...
loader <- function(i){
  load(paste0('~/Downloads/binests-',i,'.RData'))
...
targets <- data.frame(
  name = unique(df$name),
  target = c(0.5, 0, 0.1, 1, 0.1, 0.33, -0.5, 0.5, 0.05, -0.3)
)
...
df.all %>% 
  ...
  theme_bw()
ggsave('~/Downloads/bin-vs-inits.png')
