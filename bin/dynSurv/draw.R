...
Omega.draw <- function(fit){
  ...
  return(list(
    D = D, beta = beta, gamma = gamma, eta = eta
  ))
}

# Draw b ------------------------------------------------------------------
b.draw <- function(b0, X, Y, Z, beta, D, Delta, K, Fi, l0i, KK, Fu, haz, gamma, eta){
  b.hat <- ucminf::ucminf(b0,
                          joint_density, joint_density_ddb,
                          X, Y, Z, beta, D, Delta, K, Fi, l0i, KK, Fu, haz, gamma, eta)$par
  Sigmai <- solve(joint_density_sdb(b.hat, X, Y, Z, beta, D, Delta, 
                                    K, Fi, l0i,
