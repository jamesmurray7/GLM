...
#' Should simulate data under K = 1+ responses, which will ALL be nbinom
#' #######

simData <- function(n, ntms, beta, D, gamma, eta, disp.param = 1.5,
  ...
  dfs <- list(); 
  for(i in 1:n){
    Yijk <- mu_ijks <- matrix(NA, nr = length(time), nc = nK) # i sub; j time; k longit response
    for(k in 1:nK){
      mu_ijks[, k] <- exp(cbind(1, time, cont[i], bin[i]) %*% beta[k, ] + cbind(1, time) %*% b[i, qk[[k]]])
      Yijk[, k] <- MASS::rnegbin(length(time), mu = mu_ijks[, k], theta = disp.param)
    }
