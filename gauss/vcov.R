...
vcov <- function(Omega, data.mat, b, Sigmai, l0u, gh.nodes, n){
  ...
  # beta and residual variance
  EXb <- mapply(function(X, Z, b) X %*% beta + Z %*% b, X = X, Z = Z, b = b)
  tau <- mapply(function(Z, S) unname(sqrt(diag(tcrossprod(Z %*% S, Z)))), Z = Z, S = Sigmai)
  V <- lapply(m, function(v) diag(var.e, nr = v, nc = v))
  
  Sb <- mapply(function(X, V, Y, Z, b, tau){
    rhs <- 0
    for(l in 1:gh.nodes) rhs <- rhs + w[l] * v[l] * tau
    crossprod(X, solve(V) %*% (Y - X %*% beta - Z %*% b - rhs))
  }, X = X, V = V, Y = Y, Z = Z, b = b, tau = tau, SIMPLIFY = F)
  ...
  for(i in 1:n){
    rhs <- 0
    for(l in 1:gh.nodes){
        rhs <- rhs + w[l] * crossprod(Y[[i]] - EXb[[i]] - v[l] * tau[[i]])
    } 
    Ss[[i]] <- -m[[i]]/(2*var.e) + 1/(2 * var.e^2) * rhs
  }
